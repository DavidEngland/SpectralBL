...
Within our Riemannian manifold space, the active spectral degrees of freedom provide an empirical diagnostic of how strongly multi-scale cascade structures are represented in the reconstructed column. Let $\Delta z_{\mathrm{local}} = J(\xi) \Delta \xi$ represent the local spatial grid resolution at height $z$. We interpret decreases in $D_{\mathrm{eff}}$ as informational compression when buoyancy forces constrain physical eddy scales below features resolvable by the discrete basis.

Multi-scale cascade processes are expected to scale with the active spectrum. The upper spectral boundary $n \approx D_{\mathrm{eff}}$ tracks an effective cutoff wavenumber $k_{\max} \sim D_{\mathrm{eff}} / z_{\max}$. By invoking an isotropic mapping where the minimum allowable physical grid spacing matches the local Ozmidov limit ($\Delta z_{\mathrm{local}} \sim L_O$), we obtain a phenomenological scaling relationship between the information-theoretic metric and the underlying stratified fluid mechanics:
\begin{equation}
...
When $L_O \rightarrow \infty$ (the unstratified neutral limit), the physical restriction lifts, allowing $D_{\mathrm{eff}}$ to approach the maximum data-supported rank ($r_{\mathrm{eff}}$). In this regime, buoyancy no longer constrains the vertical eddy spectrum, and broad inertial-range eddies can spread energy across the available spectral support, consistent with high-$D_{\mathrm{eff}}$ projections observed in Regime~1 of the CASES-99 analysis. Conversely, during highly stable gravity-wave saturation events where $\epsilon \rightarrow 0$ and $N_b$ peaks, $L_O$ drops below the grid's generative threshold, and $D_{\mathrm{eff}}$ typically contracts toward low-rank values. This scaling provides a physically interpretable consistency check linking entropy-based diagnostics to turbulence-state constraints highlighted by \citet{Garanaik2019}.
...
\subsubsection{Approximate Invariance Across Data Densities}
\label{sec:extensions:invariance}

A critical requirement of geophysical state estimation is that extracted information-theoretic signatures remain approximately invariant to sampling density $M$ over practical observing ranges. If the calculated effective modal diagnostic ($D_{\mathrm{eff}}$) scaled strongly with the number of operational sensors, it would fail as a robust physical diagnostic.
...
Consequently, if the underlying physical boundary layer collapses into a strongly stratified, gravity-wave dominated state, the energy distribution within the spectral coefficient vector $\mathbf{c}$ remains tightly constrained to lower-order modes. The Shannon entropy calculation (Eq.~\ref{eq:d_eff_def}) is expected to converge toward a low-rank value in the regenerated CASES-99 range (roughly $D_{\mathrm{eff}} \approx 1.1$--$1.7$), with only limited sensitivity to whether the field was captured by a handful of physical instruments or an intensive laser array. This approximate invariance supports treating the extracted features as transferable diagnostics for multi-instrument geophysical validation.
