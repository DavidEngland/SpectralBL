...
function clean_tex_comment_string(s::AbstractString)
    out = replace(s, "&" => "and")
    out = replace(out, "%" => "percent")
    out = replace(out, "\\" => "/")
    out = replace(out, "\n" => " ")
    return strip(out)
end

function normalize_transform_label(s::AbstractString)
    out = replace(s,
        "α" => "alpha",
        "β" => "beta",
        "γ" => "gamma",
...
function export_transform_reporting(ws, output_dir::String, generated_dir::String, run_suffix::String="")
    ...
    transform_df = DataFrame(
        Parameter = ["map_type"; [normalize_transform_label(p.first) for p in param_pairs]],
...
"""
    export_manuscript_transformation_section(active_map::UnifiedManifold.Transforms.CoordinateMap;
                                             output_path::AbstractString)

Writes a mathematically rigorous, differential-geometric LaTeX snippet describing
the vertical manifold metric, operator matrices, and metric-consistent quadrature.
"""
function export_manuscript_transformation_section(active_map::UnifiedManifold.Transforms.CoordinateMap;
        output_path::AbstractString=joinpath("drafts", "sections", "generated", "transformations_section.tex"))
    mkpath(dirname(output_path))
    clean_map_name = clean_tex_comment_string(UnifiedManifold.Transforms.map_name(active_map))

    tex_content = """
% ==============================================================================
% AUTOMATICALLY GENERATED BY SPECTRALBL REPORT PIPELINE
% GENERATED ON: $(Dates.now(Dates.UTC))
% ACTIVE COMPUTATIONAL MAP OBJECT: $(clean_map_name) (Riemannian Vertical Manifold)
...
$(UnifiedManifold.Transforms.to_latex(active_map))
...
"""
    ...
    println("✓ Pure LaTeX transform section written to: ", output_path)
    return output_path
end
...
function run_diagnostic_pipeline(output_dir::String)
    ...
    export_manuscript_transformation_section(ws.map)
    ...
    p1 = plot(ws.xi_target, ws.z_atm, marker=:circle, linewidth=2,
              ...
              left_margin=16Plots.mm, bottom_margin=12Plots.mm)

    p2 = plot(0:ws.N, hcat(ws.psi_M, ws.psi_W, ws.psi_T), linewidth=2.5,
              title="Modal Partitioning Windows",
              xlabel=L"n", ylabel=L"\psi",
              label=[L"\psi_M" L"\psi_W" L"\psi_T"], legend=:topright,
              left_margin=16Plots.mm, bottom_margin=12Plots.mm)

    p3 = plot(ws.z_atm, hcat(ws.psi_M_z, ws.psi_W_z, ws.psi_T_z), linewidth=2.5,
    ...
    combined_plot = plot(p1, p2, p3, layout=(1, 3), size=(1760, 520), left_margin=16Plots.mm, bottom_margin=12Plots.mm)
    ...
    cond_number = cond(ws.Manifold_Mass)
        dz_descending = [ws.z_atm[i] - ws.z_atm[i+1] for i in 1:ws.N]
        min_dz = minimum(dz_descending)
        max_dz = maximum(dz_descending)
...
- Spectral expansion order (N): $(N)
- Physical column extent: $(z_0m) m <= z <= $(z_top) m
- Stretching parameter (alpha_s): $(alpha_stretch)
- Metric mass matrix condition number (kappa(M)): $(@sprintf("%.4e", cond_number))
- Grid cell spacing limits (descending node traversal):
    - Minimum vertical grid spacing: $(@sprintf("%.3f", min_dz)) m
    - Maximum vertical grid spacing: $(@sprintf("%.3f", max_dz)) m

All system diagnostic assertions are checked and confirmed.
"""
